...
constexpr int PREFETCH_DEGREE = 10 ; //@nishkarsh:constexpr-meaning variable is availabe to be evaluated at runtime
...
//@nishkarsh:what happens every cycle of processor
void CACHE::prefetcher_cycle_operate()
{
  // If a lookahead is active
  //@nishkarsh:what do we mean by lookahead is still active
  if (auto [old_pf_address, stride, degree] = lookahead[this]; degree > 0) {
    ...
    if (virtual_prefetch || (pf_address >> LOG2_PAGE_SIZE) == (old_pf_address >> LOG2_PAGE_SIZE)) {   //@nishkarsh: the next page might not be contiguously allocated
      ...
      if (success)
      //@nishkarsh: this means even if degree>0, in one cycle we are fetching only one pf_address
      //@nishkarsh: generating the new lookahead in this cycle
...
//@nishkarsh:checking cache with the ip of the instruction
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in)
{
  //cout<<"i am inside this";
  uint64_t cl_addr = addr >> LOG2_BLOCK_SIZE;
  int64_t stride = 0;
  //@nishkarsh:is the ip a VA or PA??
  ...
  //@nishkarsh:are we checking if this cl_addr is present in the cache
